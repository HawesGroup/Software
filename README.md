Please see the releases folder for packages.

## VTAgentAssist
A desktop app to display messages and information to an agent after from processing the audio through VoizTrail's AI engine.

#### 1.0.2 2025-11-12
* Add support for UDF1 attribute to be rendered in the popup. Example use case, Comment Code field in ARTrail and SucceedAR
* Correct a window sizing issue.
	
#### 1.0.1 2025-11-03
* Add message acknowledgement feature to ensure user is aware if a submission failed. 
* Dynamic port assignment for callback listener
* Fixes to updater query logic

#### 1.0.0 2025-10-28
* Initial Release

